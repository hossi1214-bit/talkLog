...
実際の録音音声ファイルはStorageに作成しません。そのため、履歴再生の確認ではなく、スクリーンショット撮影用の表示データとして使います。アプリ側では `local_audio_path = screenshot-demo` の録音をスクリーンショット用の表示データとして扱います。
